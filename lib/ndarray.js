...
class Ndarray {
    constructor(shape = [], dtype = 'float32', initializer = 'zeros', arr = []) {
        this.array = arr.length === 0 ? core.fill(shape, initializer) : arr;
    ...
    fill(initializer, ...args) {
        this.array = core.fill(this.shape, initializer, ...args);
        this.flatten();
    }
