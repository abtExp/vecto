...
class Ndarray extends Array {
    constructor({ shape = [], dtype = 'float32', initializer = 'zeros', arr = [] }, ...args) {
        ...
        super(...array);
        this.shape = ((shape.length) > 0) ? shape : (core.calc_shape(array));
        this.size = core.calc_size(array);
        this.dim = core.find_dim(array);
        this.dtype = dtype;
        this.flat = core.form_arr(core.flatten(array), this.dtype);
    }
